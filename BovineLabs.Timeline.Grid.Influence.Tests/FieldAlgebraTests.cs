...
namespace BovineLabs.Timeline.Grid.Influence.Tests
{
    ...
    {
        static readonly GridSpec Spec = GridSpec.FromPowerOfTwo(2, uint.MaxValue);
        ...
        public void SuperpositionHoldsForArbitraryScenes()
        {
            for (int seed = 1; seed <= 120; seed++)
            {
                ...
                int[,] fl = InfluenceTestHarness.Run(Spec, left, min, size);
                int[,] fr = InfluenceTestHarness.Run(Spec, right, min, size);
                int[,] fc = InfluenceTestHarness.Run(Spec, combined, min, size);
        ...
        public void StampAndItsNegationCancelToZero()
        {
            for (int seed = 1; seed <= 200; seed++)
            {
                ...
                int[,] field = InfluenceTestHarness.Run(Spec, scene, min, size);

                foreach (int value in field)
                {
                    Assert.AreEqual(0, value, $"cancellation seed {seed}");
                }
            }
        }
        ...
        public void TranslationShiftsFieldExactly()
        {
            for (int seed = 1; seed <= 120; seed++)
            {
                ...
                Stamp stamp = InfluenceTestHarness.RandomStamp(ref rng);
                int2 delta = rng.NextInt2(new int2(-20, -20), new int2(21, 21));
                ...
                int[,] f0 = InfluenceTestHarness.Run(Spec, baseline, min, size);
                int[,] f1 = InfluenceTestHarness.Run(Spec, shifted, min + delta, size);
        ...
        public void WeightScalesLinearly()
        {
            for (int seed = 1; seed <= 120; seed++)
            {
                ...
                Stamp once = InfluenceTestHarness.RandomStamp(ref rng);
        ...
        public void RepeatedRunsAreDeterministic()
        {
            ...
            int[,] first = InfluenceTestHarness.Run(Spec, scene, min, size);
            for (int i = 0; i < 8; i++)
            {
                CollectionAssert.AreEqual(first, InfluenceTestHarness.Run(Spec, scene, min, size), $"determinism run {i}");
            }
        }

        static Stamp[] NewScene(ref Random rng, int count)
        {
        ...
        static Stamp[] Concat(Stamp[] a, Stamp[] b)
        {
