...
- **Decay / spread are per-tick, not per-second.** The diffusion stencil is applied once per resolved tick, so on a variable-rate group the diffusion speed and the equilibrium magnitude of a continuously-stamped decaying field change with frame rate. Reason in ticks. Set the `influencefield.tick-interval` config var (seconds; default `0` = tick every frame) to run the field on a fixed cadence — otherwise two machines at different FPS will read different numbers.
